player setVariable ["grad_welder_active", true, true];

private _identifier = format ["grad_welder_%1_%2", position player, CBA_missionTime];
[_identifier, player] remoteExec ["grad_welder_fnc_activateClients"];
...
[{
	...
	if (!(player getVariable ["grad_welder_active", false])) exitWith {
	...
	if (typeOf _intersectObject == "land_gm_tanktrap_02") then {
		...
		private _healthPerFrame = 0.005;
		private _health = _intersectObject getVariable ["grad_welder_tanktrap_health", 1];
		private _newHealth = _health - _healthPerFrame;
		// systemChat str _newHealth;
		_intersectObject setVariable ["grad_welder_tanktrap_health", _newHealth];

		if (_newHealth < 0.1) exitWith {
			
			[getPos _intersectObject] call grad_welder_fnc_trapDestroy;
		...
		private _soundActive = player getVariable ["grad_welder_soundActive", objNull];
		...
		if (isNull _soundActive) then {
			...
			player setVariable ["grad_welder_soundActive", _activeSound];
		};
