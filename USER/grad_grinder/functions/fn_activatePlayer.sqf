...
[{
	...
	if (typeOf _intersectObject == "SpaceshipCapsule_01_F") then {

		private _healthPerFrame = 0.005;
		private _health = _intersectObject getVariable ["grad_grinder_health", 1];
		private _newHealth = _health - _healthPerFrame;
		// systemChat str _newHealth;
		_intersectObject setVariable ["grad_grinder_health", _newHealth];
